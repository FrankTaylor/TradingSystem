 ...
 * @version 1.0
 */
public class TestOrderInfoRepositoryImpl implements TestOrderInfoRepository {
	...
	public void createIndex (String stockCode) {
		...
		try {
			 ...
			 * 原生语句：db.test_orderInfo_123456.ensureIndex({"tradeDate" : 1, "tradeTime" : 1}, {"unique" : true, "dropDups" : true, "sparse" : true, "name" : "tradeDate@tradeTime", "background" : 1})
			 */
	...
	public OrderInfoPO findNewOne (String stockCode) {
		...
		try {
			/*
			 * 原生语句：db.test_orderInfo_123456.find().sort({tradeDate : -1, tradeTime : -1}).limit(1);
			 */
