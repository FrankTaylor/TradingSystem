...
public class TestMonitor implements Runnable {
	...
	/** 日志。*/
	private static final Logger log = LogManager.getLogger(TestMonitor.class);
	 ...
	 */
	public TestMonitor (Integer taskNums, AtomicInteger completeTaskNums) {
	...
	public void run() {
		...
		try {
			while (!current.isInterrupted()) {
				...
				log.info("当前交易系统测试的进度为：" + rate.floatValue() + "%");
				TimeUnit.MILLISECONDS.sleep(10000);
			}
		} catch (InterruptedException e) {
			log.warn("[name = " + name + "]被挂起，可能已经完成了交易系统测试任务！");
		} catch (Exception e) {
			log.error("[name = " + name + "]在监控交易系统测试的过程中出现错误！");
			log.error(e);
		}
	}
