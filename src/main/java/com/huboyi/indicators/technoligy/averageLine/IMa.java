 ...
 * 简单移动平均。
 *
 * Created by hubin3 on 2016/7/25.
 */
public class IMa implements Ma {
     ...
     * @param num 取值个数
     */
    public IMa(IStatistics statistics, Period period, IStatisticsDataType dataType, int step, int num) {
     ...
     * @param num 取值个数
     */
    public IMa(Double[] vs, int step, int num) { this(vs, step, num, DEFAULT_SCALE); }
     ...
     * @param scale 精度
     */
    public IMa(Double[] vs, int step, int num, int scale) {
    ...
    private Double[] compute(final Double[] vs) {
        Double[] temp = QuoteFunction.ma(vs, step);
