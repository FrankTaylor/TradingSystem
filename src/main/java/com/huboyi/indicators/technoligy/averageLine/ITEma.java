 ...
 * 三重指数移动平均线。
 *
 * Created by hubin3 on 2016/8/12.
 */
public class ITEma implements Ma {
     ...
     * @param num 取值个数
     */
    public ITEma(IStatistics statistics, Period period, IStatisticsDataType dataType, int step, int num) {
     ...
     * @param num 取值个数
     */
    public ITEma(Double[] vs, int step, int num) { this(vs, step, num, DEFAULT_SCALE); }
     ...
     * @param scale 精度
     */
    public ITEma(Double[] vs, int step, int num, int scale) {
    ...
    private void init() {
        ...
        int range = (step * 4) + ((num - step) > 0 ? (num - step) : 0);                                    // 取值范围。
    ...
    private Double[] compute(final Double[] vs) {
        Double[] temp = QuoteFunction.tema(vs, step);
