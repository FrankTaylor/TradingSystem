 ...
 */
public class ISma implements Ma {
     ...
     * @param num 取值个数
     */
    public ISma(IStatistics statistics, Period period, IStatisticsDataType dataType, int step, int weight, int num) {
     ...
     * @param num 取值个数
     */
    public ISma(Double[] vs, int step, int weight, int num) { this(vs, step, weight, num, DEFAULT_SCALE); }
     ...
     * @param scale 精度
     */
    public ISma(Double[] vs, int step, int weight, int num, int scale) {
    ...
    private void init() {
        ...
        int range = Collections.max(Arrays.asList(step, num)) * 2 - 1;                                     // 由于当前 SMA 依赖于前一个 SMA 的值，为此需扩大取值范围。
    ...
    private Double[] compute(final Double[] vs) {
        Double[] temp = QuoteFunction.smaOfTaLib(vs, step, weight);
