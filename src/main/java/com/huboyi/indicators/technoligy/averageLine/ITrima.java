 ...
 * 三角形移动平均。
 *
 * Created by hubin3 on 2016/8/15.
 */
public class ITrima implements Ma {
     ...
     * @param num 取值个数
     */
    public ITrima(IStatistics statistics, Period period, IStatisticsDataType dataType, int step, int num) {
     ...
     * @param num 取值个数
     */
    public ITrima(Double[] vs, int step, int num) { this(vs, step, num, DEFAULT_SCALE); }
     ...
     * @param scale 精度
     */
    public ITrima(Double[] vs, int step, int num, int scale) {
    ...
    private Double[] compute(final Double[] vs) {
        Double[] temp = QuoteFunction.trima(vs, step);
