 ...
 * 考夫曼自适应移动平均。
 *
 * Created by hubin3 on 2016/8/9.
 */
public class IKama implements Ma {
     ...
     * @param num 取值个数
     */
    public IKama(IStatistics statistics, Period period, IStatisticsDataType dataType, int step, int num) {
     ...
     * @param num 取值个数
     */
    public IKama(Double[] vs, int step, int num) { this(vs, step, num, DEFAULT_SCALE); }
     ...
     * @param scale 精度
     */
    public IKama(Double[] vs, int step, int num, int scale) {
    ...
    private void init() {
        ...
        int range = Collections.max(Arrays.asList(step, num)) * 2 - 1;                                     // 取值范围。
    ...
    private Double[] compute(final Double[] vs) {
        Double[] temp = QuoteFunction.kama(vs, step);
