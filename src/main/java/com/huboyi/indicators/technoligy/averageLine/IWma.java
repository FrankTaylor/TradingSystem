 ...
 * 加权移动平均线。
 *
 * Created by hubin3 on 2016/8/8.
 */
public class IWma implements Ma {
     ...
     * @param num 取值个数
     */
    public IWma(IStatistics statistics, Period period, IStatisticsDataType dataType, int step, int num) {
     ...
     * @param num 取值个数
     */
    public IWma(Double[] vs, int step, int num) { this(vs, step, num, DEFAULT_SCALE); }
     ...
     * @param scale 精度
     */
    public IWma(Double[] vs, int step, int num, int scale) {
    ...
    private Double[] compute(final Double[] vs) {
        Double[] temp = QuoteFunction.wma(vs, step);
