...
public class IKd {
     ...
     * @param num 取值个数
     */
    public IKd(IStatistics statistics, Period period, int step, int kStep, int dStep, int num) {
     ...
     * @param num 取值个数
     */
    public IKd(Double[] hs, Double[] ls, Double[] cs, int step, int kStep, int dStep, int num) {
     ...
     * @param scale 精度
     */
    public IKd(Double[] hs, Double[] ls, Double[] cs, int step, int kStep, int dStep, int num, int scale) {
    ...
    private void init() {
        ...
        if (!ArrayUtils.isEmpty(temp) && temp.length == 2) { ks = temp[0]; ds = temp[1]; }
    }
    ...
    private Double[][] compute(final Double[] hs, final Double[] ls, final Double[] cs) {
        ...
        return new Double[][]{
                TechnoligyTools.reverseAndSubAndScaleDoubles(
    ...
    // --- static method ---
    /**
     * 根据数据序列计算均值，返回的均值数列 “由近到远” 排列，但未对小数精度进行处理。
     *
     * @param hs 最高价数组序列（该数列必须符合 “由远到近” 方式排序）
     * @param ls 最低价数组序列（该数列必须符合 “由远到近” 方式排序）
     * @param cs 收盘价数组序列（该数列必须符合 “由远到近” 方式排序）
     ...
     * @return Double[][] [0][0]：ks；[1][0]：ds；
     */
    public static Double[][] computeKD(
            final Double[] hs, final Double[] ls, final Double[] cs,
            final int step, final int kStep, final int dStep) {

        if (step < 0 || kStep < 0 || dStep < 0) { return null; }
         ...
         */
        // --- 计算 RSV ---
        Double[] rsv = QuoteFunction.rsv(hs, ls, cs, step);
        if (ArrayUtils.isEmpty(rsv)) { return null; }

        // --- 计算 K 和 D ---
        Double[] tempKs = QuoteFunction.sma(rsv, kStep, 1);
        Double[] tempDs = QuoteFunction.sma(tempKs, dStep, 1);

        Double[][] temp = { tempKs, tempDs };
